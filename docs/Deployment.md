...
1. **Production solution source** –
   `src\CampanulaCreateConcertPlanFromTemplate` is the unpacked solution source
   packed and imported by GitHub Actions.
2. **Exported solution structure reference** –
   `src\CampanulaCreateConcertPlanFromTemplateSolution` is an exported and
   unpacked Power Platform solution sample. Use it only to understand the
   solution folder/file layout.
3. **Downloaded Flow package reference** –
   `src\CampanulaCreateConcertPlanFromTemplateDemo` was manually built in Power
   Platform and downloaded as a package. It is no longer the preferred ALM
   structure reference.

The docs, Microsoft Form definition, and Excel template are the source
specification for `CampanulaCreateConcertPlanFromTemplate`.
...
| Folder | Origin | Notes |
| --- | --- | --- |
| `src\CampanulaCreateConcertPlanFromTemplate` | Production unpacked solution source. | GitHub Actions packs and imports this folder. It can contain multiple Flow components later; currently it contains the first Flow. |
| `src\CampanulaCreateConcertPlanFromTemplateSolution` | Exported and unpacked solution sample. | Use as a structure reference only; do not migrate its content into the production folder. |
| `src\CampanulaCreateConcertPlanFromTemplateDemo` | Manually created in Power Platform and downloaded as a package. | Older package-format reference, not the solution-based ALM reference. |
| `src\CampanulaTasksFlow` | Generated by GitHub Copilot during repo initialization. | Legacy prototype; do not use as the implementation source for `CampanulaCreateConcertPlanFromTemplate`. |

Unpacked solution source structure:

```text
src\CampanulaCreateConcertPlanFromTemplate
├── [Content_Types].xml
├── customizations.xml
├── solution.xml
└── Workflows\
    └── <flow-name>-<workflow-id>.json
```

Use `src\CampanulaCreateConcertPlanFromTemplateSolution` as a sample of this
structure. Power Platform CLI solution commands expect unpacked solution source,
not a downloaded Flow package export.

---

## Build CampanulaCreateConcertPlanFromTemplate in Power Platform

Recommended workflow for creating `CampanulaCreateConcertPlanFromTemplate`:
...
```bash
pac solution unpack \
  --zipfile downloads/CampanulaCreateConcertPlanFromTemplate.zip \
  --folder  src/CampanulaCreateConcertPlanFromTemplate \
...
Use `src\CampanulaCreateConcertPlanFromTemplateSolution` as the structure
reference for unpacked solution files while building
`CampanulaCreateConcertPlanFromTemplate`. The older
`src\CampanulaCreateConcertPlanFromTemplateDemo` folder is only a downloaded
Flow-package reference.
...
├── solution.xml
└── Workflows\
    └── CampanulaCreateConcertPlanFromTemplate-<workflow-id>.json
```
...
```text
src\CampanulaCreateConcertPlanFromTemplateSolution
├── [Content_Types].xml
├── customizations.xml
├── solution.xml
└── Workflows\
...
```text
src\CampanulaCreateConcertPlanFromTemplateDemo
├── manifest.json
└── Microsoft.Flow\
    └── flows\
        ├── manifest.json
...
`src\CampanulaTasksFlow` is retained only as a legacy Copilot-generated
prototype. Do not copy from it when building `CampanulaCreateConcertPlanFromTemplate`.
...
For `CampanulaCreateConcertPlanFromTemplate` as a solution-based Flow, maintain
the solution version in the unpacked solution's `solution.xml` (`<Version>`).
